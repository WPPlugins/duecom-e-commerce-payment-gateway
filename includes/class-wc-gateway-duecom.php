...
class WC_Gateway_Duecom extends WC_Payment_Gateway_CC {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'Due.com E-Commerce Payments', 'woocommerce-gateway-duecom' );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'store_cards'     => array(
				...
				'label'       => __( 'Allow logged in customers to save credit cards for future purchases', 'woocommerce-gateway-duecom' ),
				'type'        => 'checkbox',
				'description' => '',
				'default'     => 'yes',
			),
	...
	public function checks() {
		...
		if ( version_compare( WC()->version, '2.6', '<' ) ) {
			echo '<div class="error"><p>' . sprintf( __( 'Error: Due.com E-Commerce Payment Gateway requires WooCommerce 2.6 and above. You are using version %s. Contact Due Support for further assistance.', 'woocommerce-gateway-duecom' ), WC()->version ) . '</p></div>';

			return;
		}

		if ( !function_exists('curl_version') ) {
			echo '<div class="error"><p>' . 'Error: Due.com E-Commerce Payment Gateway requires cURL to be installed.' . '</p></div>';
		...
		if ( 'yes' !== $this->sandbox && ( empty( $this->app_id ) || empty( $this->api_key ) ) ) {
			echo '<div class="error"><p>' . __( 'Error: Due.com E-Commerce Payment Gateway needs your App id and Api key!', 'woocommerce-gateway-duecom' ) . '</p></div>';
	...
	public function process_payment( $order_id ) {
        ...
        $risk_data['unstructured_order'] = (array)$order;
        foreach($order->get_items() as $item){
            $product_item = array();
            $product_item['unstructured'] = $item;
            if(!empty($item['line_total'])&&!empty($item['qty'])){
                $product_description = (
		...
		if ( $transaction && $transaction->id ) {
            ...
            //todo this logic is not currently used
	...
	public function process_refund( $order_id, $amount = NULL, $reason = '' ) {
		...
		if ( $refund && $refund->id ) {
            ...
            //todo this logic is currently not used
	...
	public function scheduled_subscription_payment( $amount_to_charge, $renewal_order ) {
		try {
            ...
            if(!empty($renewal_order->id)){
                $risk_data['renewal_order_id'] = $renewal_order->id;
            }
